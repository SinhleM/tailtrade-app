...
const Dashboard = () => {
  ...
  }, [navigate]); // Removed activeSection from here, it's handled in the next useEffect
  ...
  }, [activeSection, currentUser]); // Added currentUser dependency
  ...
  const handleManageListing = async (listingId, listingType, action) => {
    // Added confirmation for delete and revert actions
    const confirmAction = action === 'delete' ? 'delete this listing' : (action === 'revert' ? 'revert this listing to active' : '');
    if (confirmAction && !window.confirm(`Are you sure you want to ${confirmAction}?`)) {
        return;
    }
    ...
    try {
      ...
      const body = { listingId, listingType }; //
      if (action === 'markSold') {
        body.status = 'sold'; //
      } else if (action === 'revert') {
        body.status = 'available'; // Or whatever your default status is
      }
