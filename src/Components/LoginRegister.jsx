...
import { testConnection } from '../utils/api'; // Import the testConnection function
...
const LoginRegister = () => {
  ...
  // New state for connection test result
  ...
  // Function to run the CORS OPTIONS test
  const runCorsTest = async () => {
      console.log('Running CORS OPTIONS test...');
      const success = await testConnection();
      if (success) {
          setTestResult('CORS OPTIONS test: PASSED (check console for headers)');
      } else {
          setTestResult('CORS OPTIONS test: FAILED (check console for errors)');
      }
  };
